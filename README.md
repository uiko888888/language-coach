# Language Coach

一个本地优先、面向语境学习与考试训练的语言学习工作台。

Language Coach 将文章阅读、词典查询、生词积累、一文多练、错题讲解和网页划词收集放在同一套学习流程中。当前版本优先强化英语，支持 IELTS、TOEFL、四级、六级、考研英语、专四、专八、GRE、GMAT 等训练风格；法语和西班牙语内容仍在后续规划中。

> 当前版本：`0.8.0-alpha.24.2`。项目仍处于 Alpha 阶段，数据结构和交互可能继续调整。

## 核心功能

### 两种学习模式

- 左侧“用户中心”集中展示画像基线、分项能力、校准状态、目标、兴趣、内容偏好和每日计划；用户可以随时主动修改。
- 用户填写的目标与偏好不会被自动校准覆盖；七天校准只依据合格训练证据调整对应能力分项。
- 首次打开时通过弹窗建立学习画像；保存后提供一次快速上手助手，之后可从左侧栏随时重新打开画像或助手。
- 顶部可随时切换兴趣模式与备考模式，选择保存在本机。
- 兴趣模式优先订阅、新闻、文化和较短内容；备考模式优先当前考试匹配与深度训练价值。
- 当前模式已影响“今日内容”，但尚未接入完整动态画像或能力预测。
- 每日计划可选择 5/15/30/60 分钟，自由组合阅读、主动输出、做题、错题复盘和词块复习，并为每项设置数量目标。
- 阅读词数、输出句数和已复习词块数使用独立目标与统计；这些完成量不直接提高能力或 CEFR。
- 今日页显示完成量、百分比和预计剩余时间；文章、错题和插件素材可加入队列，做题、解决错题和新增生词会自动更新进度。
- 用户可关闭画像推荐并退回通用内容安排；系统级通知和跨设备推送尚未实现。

### 文章与阅读

- 内容中心按新闻、观点、研究、科学与自然、文化与生活、影视与听力、小说与图书及我的订阅筛选，不把媒体名称堆进左侧导航。
- 来源注册表记录内容分类、接入方式、版权处理、更新频率、内容形式、难度和字幕/讲稿可用性；用户可订阅单个来源或整个分类。
- 自动更新仅用于许可和技术条件明确的 RSS/API。付费媒体、流媒体和学校内部内容只保存元数据、原站链接或用户授权摘录。
- 文章池采用左侧标题列表、右侧内容详情的工作台布局。
- 按来源、主题、难度和考试适配度筛选文章，并生成每日推荐。
- 应用启动时异步检查 RSS 新鲜度，服务运行期间每 6 小时刷新；支持手动刷新、HTTP 缓存、失败退避、三级去重和来源健康状态。
- RSS 文章保存真实发布时间；推荐新鲜度使用发布时间，不把重新导入时间冒充新闻发布时间。
- 支持国际时政、公共政策、商业、科学和公共机构来源，并区分新闻报道、观点评论、学术解释、研究摘要、机构公告和文化内容。
- 来源注册表区分 RSS 自动更新、摘要跳转、浏览器授权摘录、本地字幕和公共领域全文；支持来源订阅和 5/15/30 分钟今日内容。
- 区分 RSS 摘要与完整正文，避免把摘要误当作原文。
- The Conversation 自动正文会把作者、图片说明和披露声明移出阅读正文，并显示提取版本、置信度和人工纠错入口。
- BBC、Guardian、JSTOR Daily 与 The Conversation 使用独立、版本化来源适配器；反馈量和分类器数据门槛可见，正文变化保留审计。
- 来源信息可打开桌面区块标注台，把审计原文逐块确认成正文、作者、图片说明、披露、订阅噪声或不确定；真实标签才进入分类器门槛。
- 可启动 20 篇代表性抽检批次，在素材足够时平衡 BBC、The Conversation、Guardian 和 JSTOR；支持断点续标，并统计有效耗时、建议命中率、纠正混淆和来源差异。
- 原文始终保留；中文译文可按需显示、编辑和保存。
- 支持按本地路径导入私人 EPUB：先进入独立书库并按 spine 分章，选择章节后才送入阅读台，避免整本书淹没文章池。
- EPUB 原文件和正文只保存在本机数据库，不提交 Git；应用不提供受版权保护书籍的分发或共享。
- 支持文章一键翻译、分段缓存和逐段原文/译文对照；未配置翻译服务时可手动按段添加。
- WordNet 英文释义和例句可按需批量翻译，中文逐条显示在对应英文下方并缓存；应用会区分 DeepL 已配置、已验证和验证失败。
- 查词会在已验证翻译服务可用时自动补充中文，不需要再手动点击“翻译中文义项”。
- WordNet 词条会显示单词直译、逐义项释义、近义词、反义词、语义关系和来自个人文章的可追溯搭配；没有数据时会明确标记来源缺失。
- 页面导航会同步浏览器历史，支持返回上一页；练习中心会自动载入或生成当前文章题目。
- 练习中心区分专项练习、单篇组合和整套模拟；反馈方式可独立选择训练模式或模考模式。
- 考试题型研究、版权边界、公开样题来源和模型路线记录在 `EXAM_QUESTION_RESEARCH.md`。
- IELTS 阅读已具备判断题、段落标题匹配、段落信息匹配和摘要填空四类独立模板；新题入库前校验证据、答案、选项和题型规则。
- 作答会记录题型、能力与结构化错因，练习中心只加载当前考试和专项的新题，避免历史通用题混入。
- 自动整理段落、移除重复标题，并识别环境、健康、科技、历史等主题。
- 阅读时可点词查询，并将单词连同原句保存到生词本。

### 词汇中心

- 同一单词的不同词性合并在一张词条页中，每个义项单独标注词性，并在例句前先显示对应中文释义。
- WordNet 开放例句和个人文章真实语境都支持多例展示；翻译服务可批量补齐义项、例句、语义关系、搭配和个人语境的中文并缓存。
- 人工精选搭配与个人语料抽取严格区分；个人搭配按实际出现次数排序，不冒充通用语料频率。
- 中文来源会区分开放双语数据与机器翻译。牛津、剑桥、柯林斯等商业词典不抓取内置，只能使用正式授权接口或外部链接。
- 支持英文、中文、词形、词根、词缀和拉丁词源混合查询。
- 展示英美音标与发音、构词拆解、词源、词族和派生关系。
- 展示双语搭配、近义词组、反义词组与真实语境例句。
- 自动高亮查询词、词形变化和可识别的派生词。
- 生词保存语境，而不只保存孤立释义。
- 生词本区分单词与短语；文章分析按当前临时等级、词典等级和生词记录推荐可能不认识的表达。
- 任意英文单词或最多六词短语即使未被本地词典收录，也能继续翻译、保存、查看个人文章语境或跳转欧路。
- 生词卡可重新进入查询并返回来源文章；重复保存会更新原卡片而不是创建副本。
- 已导入 Open English WordNet 2025：提供英文义项、音标、例句、同义词和上下位语义关系，并显示 CC BY 4.0 来源。

### 记忆复习

- 单词、短语和已掌握错题进入同一到期队列，默认保持左侧列表、右侧回忆详情。
- 复习时先主动回想，再显示原句、答案和证据；用忘记、困难、记得、轻松四档评价回忆质量。
- 每次评分保存前后排期快照，十分钟内可以撤销；网络重试不能重复推进尚未到期的项目。
- 有效评分进入今日复习进度，遗忘项目进入重学并记录 lapse；生词本同步显示当前调度状态。
- 当前调度器为版本化的 `adaptive-interval-v1`，不是 FSRS；正式 FSRS 接入和个人目标保留率仍在后续版本。

FSRS 适配是可选依赖。当前 PyPI 可用的稳定版本为 `6.3.1`，不要安装不存在的 `5.2.0`：

```powershell
python -m pip install --user "fsrs==6.3.1"
python -c "import fsrs; print(fsrs.__file__); print(getattr(fsrs, '__version__', 'unknown'))"
```

安装后需先通过 API 探测、固定测试向量和旧数据迁移测试，才会启用 FSRS；未安装或校验失败时继续使用 `adaptive-interval-v1`。

### 主动输出

- 有逐段对齐译文时，同一篇文章可生成英译中理解翻译和中译英重构；所有合格长文都可生成三句英文摘要和个人表达。
- 输出工作台保持左侧原文、右侧任务与作答，可从文章池、阅读器和今日计划直接进入。
- 保存用户原答案、用时、提示使用、信心、规则反馈、结构化自评和历史，不用系统建议覆盖用户表达。
- 参考译文、原文表达和摘要要点均明确标为非唯一答案；无 AI 时仍可完成、保存和复习。
- 规则反馈只检查可确定的覆盖、数字、否定、长度、句数、复制和目标词块；自然度、语域和开放语义暂不伪装成自动评分。
- 英文目标词块、参考表达或用户编辑的任意英文错句可以进入统一复习，并保留文章来源。
- 配置 OpenAI-compatible 服务后，可按信息、搭配、语域、衔接和自然度生成五维反馈；引用必须能在原文、参考或用户答案中定位。
- 用户可以保留原表达、采纳建议或保存自己的修改；任何选择都不会覆盖原答案或直接提高能力等级。
- 内置六组高频近义词边界练习，从动作方式、搭配、语域、立场和强度区分相近表达，提交前不会下发答案。
- 口语训练支持本机录音、暂停/继续、重录、回放、删除，以及文章复述、观点表达和词块口头造句。
- 录音默认只保存在本机；只有用户主动点击转写后，音频才会发送到自己配置的转写服务。未配置转写时仍可回放、手动填写文本和自评。
- 当前文本观察只统计语速、内容覆盖、填充词、立即重复和目标词块，不提供发音评分，也不冒充 IELTS/TOEFL 官方口语评分。

AI 五维反馈是可选功能。未配置时，规则检查、自评、复习和近义词基础组仍可使用。配置示例：

```dotenv
OUTPUT_AI_PROVIDER=openai-compatible
OUTPUT_AI_API_URL=https://api.openai.com/v1/chat/completions
OUTPUT_AI_API_KEY=你的密钥
OUTPUT_AI_MODEL=你选择的模型
```

只有点击“生成五维反馈”后，当前任务原文和用户答案才会发送到所配置的服务；密钥只从本机环境读取，不写入数据库或前端。

### 练习与复盘

- 练习中心只保留反馈方式、训练范围和考试题型三个主要选择维度；单篇组合与整套模拟会分别隐藏专项题型并切换到对应工作流。
- 切换目标考试、训练范围或专项题型会立即加载匹配题目；没有现成题目时按当前文章和考试题型生成，不再残留上一题型内容。
- 同一篇文章可按考试专项生成判断、匹配、摘要填空和通用选词练习；首字母填空已移除。
- 支持 IELTS、TOEFL、四级、六级、考研英语、专四、专八、GRE、GMAT 和通用出题风格。
- IELTS 整套模拟按三篇文章和 40 题组织，当前采用 13 / 13 / 14 的规则分配，需要三篇本地完整文章。
- 考研英语已具备细节与推断、主旨与作者态度、长难句语义、完形与语境逻辑四类独立专项；英语一/二和完整套卷尚未拆分。
- TOEFL 阅读已具备 `Complete the Words` 单空模拟、事实信息、否定事实、推断、修辞目的、段落主旨、句子简化、句子插入、篇章总结和语境词义十类专项，并使用题型专属证据规则校验。
- `Complete the Words` 会保留前缀、隐藏后续字母并要求输入完整单词；错词连同原句进入生词本。当前实现是 2026 题名对应的单空规则模拟，不是 ETS 原题或官方多空界面复刻。
- 篇章总结当前是四选一训练基础，不冒充 ETS 正式六选三；完整阅读套题、官方计时与评分仍待实现。
- 做题时保留左侧原文，支持题号导航、标记、计时以及逐题/全部显示。
- 训练模式答题后立即展示答案、证据和考点讲解；模考模式统一交卷后再揭晓。
- 作答时记录“猜测 / 犹豫 / 确定”，交卷后区分真正掌握、蒙对和“确定但答错”的误解。
- 交卷结果展示正确率、完成度、用时、能力项、错因和信心校准，未作答题不会从统计中消失。
- 训练记录统一保存练习与模考，可查看单次题目、答案、证据、能力/题型正确率和最近五题趋势，并从弱项继续训练。
- 错题解析可返回阅读器并定位原文证据；“下一组 10 题”优先按未解决错题的题型和错因继续训练。
- 结果页可直接按某个错因继续练；刷新页面会恢复同一篇文章和题目集合中的未完成训练。
- 解析中的证据句支持点词查词，词汇查询可以回到题目语境。
- 题库区分官方公开链接、用户自有材料和系统模拟题，不把未授权网络真题复制进项目。
- 可针对同一考点生成巩固题，并将错题收入复盘列表。
- 使用 XP、等级和连续学习天数记录学习进度。
- 可通过已有成绩、阅读词汇快速基线或 CEFR 自评建立画像；画像记录目标、弱项和兴趣，并解释文章推荐理由。
- 兴趣模式以内容沉浸和词块积累为主，备考模式以目标、弱项和题型训练处方为主。
- 画像每七天按分项校准；只练阅读只调整阅读，综合等级需要多分项输入与输出证据。

### 浏览器扩展

- 在网页中划词查询和保存生词。
- 选中段落后请求翻译，并保存原文、译文、上下文、标题和 URL。
- 从网页右键菜单提取主要正文，导入 Language Coach 文章池。
- 插件保存的划词、摘录和正文进入首页学习队列，可继续查词、进入文章或直接生成练习。
- 本地连接使用随机令牌保护；翻译密钥只保存在后端进程环境中。

## 技术架构

```text
language-coach/
├── backend/             Python HTTP API、业务逻辑与 SQLite 持久化
├── frontend/            原生 HTML、CSS 和 JavaScript 前端
├── browser-extension/   WXT + TypeScript 浏览器扩展
├── tests/               Python 集成测试
├── data/                本地运行数据（数据库不进入 Git）
├── run.ps1              PowerShell 启动脚本
└── start-language-coach.bat
```

应用后端仅使用 Python 标准库，核心数据保存在本地 SQLite 中。浏览器扩展使用 WXT、TypeScript 和 Defuddle。

## 快速启动

### 环境要求

- Windows 10/11
- Python 3.10 或更高版本
- 构建扩展时需要 Node.js 20+ 与 pnpm

### 启动应用

克隆仓库后，在项目目录运行：

```powershell
.\run.ps1
```

也可以双击 `start-language-coach.bat`。默认访问地址：

```text
http://127.0.0.1:8765/
```

### Windows 登录后自动更新

本地版可安装当前用户登录任务。登录 30 秒后后端自动启动，随后执行启动新鲜度检查，并在运行期间每六小时检查公开来源：

```powershell
.\scripts\install_windows_autostart.ps1
.\scripts\windows_autostart_status.ps1
```

卸载只删除 Language Coach 任务，不结束其他 Python 进程：

```powershell
.\scripts\uninstall_windows_autostart.ps1
```

电脑关机、休眠或尚未登录时不会更新；下次登录会补做新鲜度检查。公共产品的长期形态是托管 Web/PWA，公共内容由云端刷新，EPUB、私有字幕和本地词典由可选本地伴侣处理，详见 [`DEPLOYMENT.md`](DEPLOYMENT.md)。

若要使用其他端口：

```powershell
python .\backend\server.py 8766
```

首次运行会自动创建 `data/language_coach.sqlite`。该数据库已被 `.gitignore` 排除。

## 构建浏览器扩展

```powershell
cd browser-extension
pnpm install
pnpm build:edge
```

构建产物位于 `browser-extension/.output/edge-mv3`。

在 Edge 的 `edge://extensions` 或 Chrome 的 `chrome://extensions` 中开启开发者模式，选择“加载解压缩的扩展”，然后指向上述目录。回到 Language Coach 的生词本页面复制本地连接令牌，并在扩展设置中保存和测试连接。

扩展也可以在开发模式下运行：

```powershell
pnpm dev
```

## 翻译配置

查词、网页正文提取和保存不需要第三方密钥。复制 `.env.example` 为不会提交到 Git 的 `.env.local`。DeepL API Free 配置示例：

```dotenv
TRANSLATION_PROVIDER=deepl
DEEPL_API_KEY=your-deepl-api-key
```

免费自托管方案可使用 LibreTranslate：

```dotenv
TRANSLATION_PROVIDER=libretranslate
LIBRETRANSLATE_URL=http://127.0.0.1:5000
```

修改后重启服务。翻译结果按文本、语言和服务商缓存在本地 SQLite 中。API Key 不会写入浏览器扩展、数据库或 Git 仓库；没有可用提供商时仍可手动逐段添加译文。

## 测试

运行后端与学习流程测试：

```powershell
python -m unittest discover -s tests -v
```

本地服务启动后，可使用捆绑或已安装的 Playwright 运行练习中心浏览器冒烟测试：

```powershell
node .\scripts\verify_practice_ui.cjs http://127.0.0.1:8765/
node .\scripts\verify_article_extraction_ui.cjs
```

检查扩展类型并构建 Edge 版本：

```powershell
cd browser-extension
pnpm typecheck
pnpm build:edge
```

## 词典与内容来源

### 导入 Open English WordNet

项目提供可重复运行的导入器。先准备官方 2025 JSON ZIP，再运行：

```powershell
python .\scripts\import_wordnet.py --zip .\artifacts\english-wordnet-2025-json.zip
```

也可以让导入器下载官方版本：

```powershell
python .\scripts\import_wordnet.py --download
```

导入结果写入本地 SQLite，不会把词典压缩包或生成的数据库提交到 Git。查询页会显示 Open English WordNet 的英文义项、例句、语义关系、版本和 `CC BY 4.0` 来源信息；中文释义仍通过 DeepL 或手动译文补充。

当前仓库内置的是用于验证学习闭环的结构化示例词条，不是对 Oxford、Cambridge 或 Collins 等商业词典的复制。后续词典扩充优先采用许可清晰的 Wiktionary/Kaikki、WordNet、Tatoeba 等开放数据；商业词典仅通过授权 API 或外部链接接入。

文章来源会记录标题、来源、原始地址和内容完整性状态。RSS 只提供摘要时，界面会明确标记并引导打开原始来源，不会伪装成完整文章。

## 第三方项目边界

浏览器扩展采用以下开源基础：

- [WXT](https://github.com/wxt-dev/wxt)：MIT License
- [Defuddle](https://github.com/kepano/defuddle)：MIT License

[Read Frog（陪读蛙）](https://github.com/mengxi-ream/read-frog)用于产品与扩展架构评估。Language Coach 没有复制其 GPL-3.0 业务代码。完整说明见 [`browser-extension/THIRD_PARTY_NOTICES.md`](browser-extension/THIRD_PARTY_NOTICES.md)。

## 版本管理

项目使用 Semantic Versioning，并通过 Git 标签保留发布节点：

- 稳定版本：`vMAJOR.MINOR.PATCH`
- 预发布版本：`vMAJOR.MINOR.PATCH-alpha.N`
- 提交信息：遵循 Conventional Commits，例如 `feat:`、`fix:`、`docs:`、`test:`、`chore:`

版本历史见 [`CHANGELOG.md`](CHANGELOG.md)，当前版本见 [`VERSION`](VERSION)。版本目标与验收见 [`PROJECT_PLAN.md`](PROJECT_PLAN.md)，未完成任务与打钩核对见 [`TASKS.md`](TASKS.md)；产品定位、架构边界、路线图、真实完成状态、成熟度对照、六分提升计划、词典数据运维、迭代复盘、关键决定和相关项目调研分别见 [`PRODUCT.md`](PRODUCT.md)、[`ARCHITECTURE.md`](ARCHITECTURE.md)、[`ROADMAP.md`](ROADMAP.md)、[`STATUS.md`](STATUS.md)、[`MATURITY.md`](MATURITY.md)、[`SCORECARD.md`](SCORECARD.md)、[`DICTIONARY_DATA.md`](DICTIONARY_DATA.md)、[`REVIEWS.md`](REVIEWS.md)、[`DECISIONS.md`](DECISIONS.md) 和 [`RESEARCH.md`](RESEARCH.md)。

## 路线图

- 扩充许可清晰的开放词典数据，并标注每条释义来源。
- 增加 URL、PDF、字幕和批量文章导入。
- 建立考试套题、题型专题和难度校准体系。
- 增加听力逐词高亮、跟读和影视片段学习。
- 增加法语、西班牙语内容与相应分级体系。
- 提供 PWA 离线缓存和更完整的移动端体验。

## 数据与安全

- SQLite 数据库、扩展构建产物、依赖目录和本地密钥不会提交到 Git。
- 不要把 `DEEPL_API_KEY`、本地连接令牌或其他凭据写入源码。
- 当前仓库尚未声明项目整体开源许可证；第三方依赖继续遵循各自许可证。
