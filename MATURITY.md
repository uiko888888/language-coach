...
| Contextual output | 6.5/10 | Writing assistants and output-first language apps | Reviewed chart and argument language improves reference support, but active boundary tasks, real-model quality and repeated-output gains are unvalidated | Add boundary practice, then audit seven-day reattempts |
...
| Dictionary and word chunks | 8.3/10 local, 8.1/10 distributable | Eudic, Cambridge, Collins, Merriam-Webster | Private Oxford coverage improves this user's lookup substantially, but it cannot become bundled public coverage; 115 confusion groups remain candidates | Turn reviewed boundaries into active recall, then continue review |
| Review scheduling | 6.7/10 | Anki/FSRS | Validated FSRS now powers general memory and a dedicated exam-item card loop; long-term retention fitting and management controls remain | Collect real logs, calibrate retention, add pause/reset/new-card limits |
