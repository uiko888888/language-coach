...
- `backend/comparison_training_audit.py` is the versioned correction-task decision and revision ledger. Runtime publication fails closed to explicitly approved corrections, and `scripts/audit_comparison_training.py` verifies revision structure plus exact equality between approved and published sets.
